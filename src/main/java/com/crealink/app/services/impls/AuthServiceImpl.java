...
public class AuthServiceImpl implements AuthService {
    ...
    public SignInResponseDto signIn(SignInRequestDto request) {
        UserDto user = userService.findByEmailOrUsername(request.username()).map(userMapper::toDto).orElseThrow(UnauthorizedException::new);
        boolean isPasswordValid = PasswordUtil.checkPassword(passwordEncoder, request.password(), user.passwordSalt(), user.password());
        if(!isPasswordValid) {
